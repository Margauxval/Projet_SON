...
struct mydsp : public dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "sampler.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		sig0->fillmydspSIG0(131072, ftbl0);
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow0 = static_cast<float>(fCheckbox0) > 0.0f;
		float fSlow1 = static_cast<float>(fButton0);
		float fSlow2 = fConst1 * static_cast<float>(fEntry0) * static_cast<float>(fSlow1 > 0.0f);
		float fSlow3 = fConst2 * fSlow1;
		float fSlow4 = static_cast<float>(fHslider0);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			iRec0[0] = ((iSlow0) ? 0 : std::min<int>(131072, iRec0[1] + 1));
			ftbl0[std::max<int>(0, std::min<int>(iRec0[0], 131071))] = static_cast<float>(input0[i0]);
			float fTemp0 = ((1 - iVec0[1]) ? 0.0f : fSlow2 + fRec1[1]);
			fRec1[0] = fTemp0 - std::floor(fTemp0);
			fRec2[0] = fSlow3 + fConst3 * fRec2[1];
			output0[i0] = static_cast<FAUSTFLOAT>(fSlow4 * fRec2[0] * ftbl0[std::max<int>(0, std::min<int>(static_cast<int>(131072.0f * fRec1[0]), 131071))]);
			iVec0[1] = iVec0[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
	...
	#define FAUST_FILE_NAME "sampler.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("Sampler/Gain", fHslider0, 0.5f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDCHECKBOX("Sampler/Record", fCheckbox0);
	FAUST_ADDNUMENTRY("Sampler/freq", fEntry0, 4.4e+02f, 2e+01f, 2e+04f, 0.01f);
	FAUST_ADDBUTTON("Sampler/gate", fButton0);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, Gain, "Sampler/Gain", fHslider0, 0.5f, 0.0f, 1.0f, 0.01f) \
		p(CHECKBOX, Record, "Sampler/Record", fCheckbox0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(NUMENTRY, freq, "Sampler/freq", fEntry0, 4.4e+02f, 2e+01f, 2e+04f, 0.01f) \
		p(BUTTON, gate, "Sampler/gate", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
...
sampler::sampler() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void sampler::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void sampler::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float sampler::getParamValue(const std::string& path)
{
